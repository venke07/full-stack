...
async def call_selected_provider(provider: str, prompt: str) -> Dict[str, Any]:
    provider = provider.lower()
    if provider == "openai":
        return await _call_openai(prompt)
    if provider in {"gemini", "google"}:
        return await _call_gemini(prompt)
    if provider in {"tavily", "tavilyai"}:
...
async def model_research(req: ModelResearchReq):
    ...
    if provider:
        try:
            return await call_selected_provider(provider, req.prompt)
        except Exception as provider_error:
            last_provider_error = str(provider_error)
            logging.warning("Provider override failed (%s): %s", provider, provider_error)
            if not ENABLE_RESEARCH_PIPELINE:
                return _mock_research_response(req.prompt, provider, detail=last_provider_error)
    ...
    try:
        ...
        logging.exception("Model research request failed")
        if not ENABLE_RESEARCH_PIPELINE:
            return _mock_research_response(req.prompt, provider, detail=str(e))
